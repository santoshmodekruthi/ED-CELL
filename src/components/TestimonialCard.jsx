...
export default function TestimonialCard({ testimonial }) {
    return (
        <div className="p-6 rounded-2xl mx-4 shadow-[0_4px_20px_rgba(0,0,0,0.05)] border border-slate-100 bg-white hover:shadow-[0_4px_30px_rgba(0,0,0,0.08)] transition-all duration-200 w-80 shrink-0">
            <div className="flex gap-3">
                ...
                <div className="flex flex-col">
                    ...
                    <span className="text-xs text-slate-400 font-medium">{testimonial.handle}</span>
                </div>
            </div>
            <p className="text-sm pt-4 text-slate-600 leading-relaxed font-medium">"{testimonial.quote}"</p>
        </div>
    );
}
